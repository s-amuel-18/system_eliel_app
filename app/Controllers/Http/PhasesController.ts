...
export default class PhasesController {
  public async index({ view, params, response }: HttpContextContract) {
    ...
    return view.render("dashboard/project/phases/index", { project, phaseStatus, areas, involveds })
  ...
  public async store({ response, params, request }: HttpContextContract) {
    ...
    const phase = await Phase.create({
      ...dataPhase, projectId: params.id
    })

    const involvedObject = involveds.filter(i => i.id).reduce((obj, item) => {
  ...
  public async supervisedStore({ params, response, request }: HttpContextContract) {
